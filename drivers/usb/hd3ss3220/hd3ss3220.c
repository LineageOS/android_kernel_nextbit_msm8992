...
static struct delayed_work hd3ss3220_mode_work;
static struct i2c_adapter *i2c_bus;
static struct i2c_msg msg_read[2];
static struct i2c_msg msg_write;
static u8 msg_buf_read;
static u8 msg_buf_write[2];
...
static void write_hd3ss3220_mode(struct work_struct *work) {
	...
	msg_read[1].buf = &reg;
	msg_read[1].len = sizeof(reg);

	// Read current control register
        for (i = 0; i < I2C_RETRY_MAX; i++) {
                ret = i2c_transfer(i2c_bus, msg_read, 2);
                if (ret != 2) {
	...
	// Clear old mode and set new one
	reg &= ~(MODE_UFP | MODE_DFP);
	reg |= MODE_UFP;
	msg_buf_write[1] = reg;

	// Write the control register
	for (i = 0; i < I2C_RETRY_MAX; i++) {
		ret = i2c_transfer(i2c_bus, &msg_write, 1);
		if (ret != 1) {
...
static irqreturn_t hd3ss3220_irq_handler(int irq, void *data)
{
	...
	schedule_delayed_work(&hd3ss3220_mode_work, msecs_to_jiffies(0));
	return IRQ_HANDLED;
}
		...
		const struct i2c_device_id *id)
{
	...
	msg_buf_read = 0x0A;
	msg_read[0].flags = 0;
	msg_read[0].addr = id->driver_data;
	msg_read[0].buf = &msg_buf_read;
	msg_read[0].len = sizeof(msg_buf_read);
	msg_read[1].flags = I2C_M_RD;
	msg_read[1].addr = id->driver_data;
	msg_buf_write[0] = 0x0A;
	msg_write.flags = 0;
	msg_write.addr = id->driver_data;
	msg_write.buf = msg_buf_write;
	msg_write.len = sizeof(msg_buf_write);
	i2c_bus = client->adapter;

	// Write UFP mode to device
	INIT_DEFERRABLE_WORK(&hd3ss3220_mode_work, write_hd3ss3220_mode);
	schedule_delayed_work(&hd3ss3220_mode_work, msecs_to_jiffies(0));

	// Setup IRQ handling
	ret = of_get_named_gpio(client->dev.of_node, "ti-hd3ss3220,irq-gpio", 0);
	if (ret < 0) {
	...
	ret = request_threaded_irq(data->irq, NULL,
			hd3ss3220_irq_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
...
static void correct_i2c_device_address(void)
{
